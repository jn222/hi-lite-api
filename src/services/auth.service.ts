...
export class AuthService {
  ...
  public async signup(userData: CreateUserDto): Promise<{ cookie: string; id: number; display_name: string; email: string }> {
    ...
    const { id, display_name, email } = createUserData;

    return { cookie, id, display_name, email };
  }

  public async login(userData: LoginUserDto): Promise<{ cookie: string; id: number; display_name: string; email: string }> {
    const findUser: User = await this.user.findUnique({ where: { email: userData.email } });
    ...
    const cookie = this.createCookie(tokenData);
    const { id, display_name, email } = findUser;
